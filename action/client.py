...
def build_pr_comment_request(markdown_body: str, github_token: str, repo: str, pr_number: int) -> urllib.request.Request:
    """Builds (but does not send) the GitHub API request to post a PR
    comment. Also its own function for the same testability reason as
...
def main():
    ...
    if post_pr_comment and pr_markdown and github_token:
        ...
        if repo and pr_num:
            c_req = build_pr_comment_request(pr_markdown, github_token, repo, pr_num)
            try:
